...
> 评审修订：2026-08-29（第十六次复核，承接第十五次校正）；本版按本轮复核基线 `c171042` 重新核对。此前 `6c507f6`、`c3b2d73` 等提交、测试数量和打包报告仅保留为历史记录，当前工作区另有未提交改动，详见实际 `git status`。防偷窥/隐私遮罩/小窗等既有 UI 功能不在本方案范围内，仅包含新 API 所需的 Preload/Store 最小接入。方案已固定采用“staging 临时路径 → 不可变 Artifact Blob”和“主进程/Preload fetch SSE 代理”，并保留跨 run receipt、受管导入、持久化安全和事件游标约束；本次进一步把“可以开始实施”和“可以放行下一阶段”拆成显式门禁，仍不能把 T1/T2/2B 宣称为已完成，旧门禁记录见 `1.10`～`1.14`，当前复核见 `1.15`。本文不再新增大段设计章节。

| 元信息 | 值 |
|---|---|
| 文档版本 | `1.2.2`（本次第十六次复核修订；API 契约版本仍为 `contracts/openapi.yaml:info.version=1.0.0-test`；语义契约已随 2A 实现落地并通过 `npm run check:contracts` 校验） |
| 基线提交 | `c171042`（2026-08-29 本轮复核基线；提交后工作区仍可能包含未提交改动；实测 Python 285 项、Electron 86 项通过，Node.js `v22.23.2`，Node 24 支持仍需在目标环境复核） |
...
| 1.2.1 | 2026-08-29 | `c3b2d73` | 第十五次复核：确认 xunfei 拆分依赖完整，补齐 wordtts 包级公共导出，更新已删除旧入口的文档引用；当前工作树实测 Python 282 项、Electron 86 项通过 |
| 1.2.2 | 2026-08-29 | `c171042` | 第十六次复核：补齐已删除 TTS 门面全部 70 项导入对应的包级导出，统一历史复核口径；当前工作树实测 Python 285 项、Electron 86 项通过 |

> **历史口径说明：** `1.1`～`1.14` 中出现的旧提交、测试数量、Node 版本、构建哈希以及“当前口径/以本节为准”等文字，均属于当时复核快照；本文件现行基线只看文首元信息和 `1.15`，不得用历史快照覆盖当前工作树。
...
> 本节为发现过程存档；当时规范以 `1.6` 为准，当前工作树口径以文首元信息和 `1.15` 为准。约束等级见文首，重复论述不再拆为独立任务。
...
截至 2026-08-28 第六次复核以 HEAD `c0cc563` 为基线（历史记录），第七次复核（`6c507f6`，见 1.7）已前移；第六次时 `git status --short --untracked-files=all` 仅显示本方案文档和 4 个示例文档未跟踪，未发现 tracked 代码修改；此前 `ed0f30f` 阶段关于讯飞 Provider 单体实现的未提交修改只作为历史记录，不作为当前工作树状态。阶段 0 在开始重构前仍必须重新记录精确的提交/工作树标识，并区分方案文档、示例数据和业务改动；当前基线以文首元信息和 1.15 为准。
...
### 1.7 第七次复核纠偏（历史记录；2026-08-28 晚间，FROZEN 增补）

> 本节记录 2026-08-28 晚间对 HEAD `6c507f6` 的历史实测结果；当时与 1.6 同为 FROZEN，当前工作树不采用本节的基线口径。
...
- `BACKEND_CONTRACT_VERSION=5 / AUDIO_ALGORITHM_VERSION=8 / PARSER_VERSION=14 / PARSE_CACHE_VERSION=10` 经 `wordtts/config.py` 与 `server.py` 复核未变。
...
### 1.8 第八次复核核验及第九次后续校正（历史记录；2026-08-28 晚间）
...
### 1.11 第十二次复核补充（历史记录；2026-08-28）

> 本节记录第十二次复核；当时口径以 1.12 为准。本次继续只更新方案文档，不修改业务代码；`contracts/`、`db/`、`docs/` 仍作为待 T1 校核的落盘材料，不能用其当前自述覆盖本方案门禁。
...
### 1.12 第十三次复核：实施就绪度与阶段放行（历史记录；2026-08-28）
...
### 1.13 第十四次复核：2A 验收历史记录（2026-08-29；历史口径）

本节记录第十四次复核时的 2A 验收结果，覆盖当时 `1.9`–`1.12` 中“仍停在 T0/T1 校核”的叙述；其中的提交、测试数量和构建产物仅作历史证据，不作为当前工作树基线。当前口径见本文件元信息和第十六次复核记录。
...
- 本文档正文继续冻结为索引；实施状态以 `docs/implementation-plan.md` 的任务表和两份门禁 JSON 为准，后续复核只追加当前条目并更新元信息。

**1.13 增补（2026-08-29 晚，T8/T16 收尾后；历史记录）：** T8（Store 订阅式 workspace 进度投影）与 T16 的代码侧剩余项（拖拽导入分块暂存流式化、语音资源显式上限、内存边界说明）已完成并有回归覆盖；T11 的新逻辑（停顿快速路径优先级）以本地 Chromium 单测验证，不触真实讯飞。当时证据为 Python `309` / Electron `84`，2A gate 13 项 PASS；`v2.7.45` DMG 通过打包冒烟与签名校验。任务表状态见 `docs/implementation-plan.md`。

**1.13 增补二（2026-08-29 晚，server.py 旧引擎物理删除）：** 按产品决定，13.1 的旧 API 处置从"410 网关 + 保留实现"升级为物理删除：`server.py` 4074→646 行，旧 `SessionState` 引擎、旧生成流水线与全部旧 `/api/*` 路由处理器移除；未迁移路径由中间件统一返回 `410 API_VERSION_RETIRED`，`legacy-api-410`/`legacy-api-retirement-code` 探针 PASS，`test_desktop_server` 收敛为 6 项活跃面测试。T12 的"路由不再编排"验收以删除完成收口。删除后重新打包的 `v2.7.45` DMG（SHA-256=`694bd0dab9ccf2732953b16d0183f8c96caee2bb4031816cbcbb66c972a82316`）通过打包冒烟与签名校验。

**1.14 第十五次复核（历史记录；2026-08-29 晚）：** 当时 HEAD 为 `c3b2d73`，工作区仍有未提交改动；Python 测试 `282` 项、Electron 测试 `86` 项通过，当前 Node.js 为 `v22.23.2`，Node 24 支持仍需在目标环境复核。xunfei 拆分依赖、wordtts 包级公共导出、旧入口文档引用已完成当时修复；逻辑 smoke、OpenAPI/生成契约检查、Python 编译和 JavaScript 语法检查通过。此前 1.13 的测试数量和 DMG 哈希均保留为历史记录，不与当前工作树混用。

### 1.15 第十六次复核：当前工作树最终口径（2026-08-29）

本轮复核基线为 `c171042`；复核时工作区仍有未提交改动，本次以当前文件内容重新完成复核。Python 全量测试 `285` 项、Electron 全量测试 `86` 项通过；Node.js 当前为 `v22.23.2`，因此 Node 24 支持仍需在目标环境复核。xunfei 包级导出与 100 个历史 `XunFeiSession` 方法逐项对齐，wordtts/question_types 旧门面导出共 70 项全部可从规范包入口取得；68 个当前 Python 文件编译通过，静态未定义符号检查、OpenAPI/生成契约检查、JavaScript 语法检查和逻辑 smoke 均通过，仓库内没有已删除旧入口路径残留。此前 1.14 及更早测试数量、提交和构建报告均只作历史追溯。
...
| xunfei/ | 浏览器会话、页面脚本、接口签名、worksId、重试、下载和多人配音已按职责拆分为独立模块 | 接入其他 TTS 或外部系统时仍需继续收敛统一 Provider 契约 |
| wordtts/ | 批量合成、音频合并、切割、进度和文件持久化按职责拆分 | 音频算法修改可能影响任务状态和恢复逻辑 |
| question_types/ | 各题型解析器、注册表和内容识别按题型切片组织 | 新题型只需增加切片与测试，仍需维护统一中间模型 |
...
当前讯飞实现已落在 `xunfei/` 包中：`session.py` 管理浏览器会话，`page_actions.py`/`page_scripts.py` 管理页面交互，`submission_tracker.py` 管理 worksId 关联，`downloads.py` 管理对账下载，`generation.py` 管理提交编排；`XunfeiTTSAdapter` 只通过包级 Provider API 调用它们。后续继续补齐统一 Provider 契约和能力声明时，应保持这些边界，不让工作流引擎重新依赖页面内部方法。
...
当前 `question_types/` 已经按题型拆出多个 Parser 类，后续重点是统一它们的中间模型、错误定位和稳定 identity，而不是重新复制一层分支。中间模型必须带 schema_version、source_locator 和 identity_version，便于缓存迁移、编辑匹配和问题定位。
...
- [ ] `git rev-parse HEAD`、`git status --short --untracked-files=all` 已存档，本轮复核基线 `c171042` 可追溯；工作区未提交改动以实际命令输出为准
