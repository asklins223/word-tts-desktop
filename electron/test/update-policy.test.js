...
const { readProjectVersion } = require('../../scripts/project_version');
...
test('读取默认更新策略不依赖调用时的工作目录', () => {
    ...
    assert.equal(readUpdatePolicy().version, undefined);
    assert.equal(validateUpdatePolicy(readUpdatePolicy(), { version }).version, version);
});
...
test('发布前更新策略校验拒绝版本错配和无效规则', () => {
    assert.throws(
        () => validateUpdatePolicy({ mode: 'optional', minimumSupportedVersion: null, message: 'ok' }, { version: '02.7.46' }),
        /不是合法 SemVer/,
    );
    assert.throws(
        () => validateUpdatePolicy({ mode: 'optional', minimumSupportedVersion: '2.8.0', message: 'ok' }, { version: '2.7.46' }),
        /不能高于/,
    );
    assert.throws(
        () => validateUpdatePolicy({ mode: 'later', minimumSupportedVersion: null, message: 'ok' }, { version: '2.7.46' }),
        /optional 或 force/,
    );
    assert.throws(
        () => validateUpdatePolicy({ mode: 'optional', message: 'ok' }, { version: '2.7.46' }),
        /缺少字段.*minimumSupportedVersion/,
    );
    assert.throws(
        () => validateUpdatePolicy({ mode: 'optional', minimumSupportedVersion: null, message: 'ok', rollout: 50 }, { version: '2.7.46' }),
        /未知字段.*rollout/,
    );
    assert.throws(
        () => validateUpdatePolicy({ mode: 'optional', minimumSupportedVersion: null, message: 'ok' }, { version: '2.7.46', tag: '2.7.46' }),
        /Release tag 必须是 v<version> 格式/,
    );
    assert.throws(
        () => validateUpdatePolicy({ mode: 'optional', minimumSupportedVersion: null, message: 'ok' }),
        /必须提供构建版本/,
    );
});

test('发布版本比较同样处理预发布版本', () => {
    assert.equal(validateUpdatePolicy({
        mode: 'optional',
        minimumSupportedVersion: null,
        message: 'ok',
    }, { version: 'V2.7.46' }).version, '2.7.46');
...
test('发布元数据从实际 Windows 安装包计算大小和 SHA-512', () => {
    ...
    try {
        ...
        const metadata = JSON.parse(fs.readFileSync(path.join(releaseDir, result.filename), 'utf8'));

        assert.equal(result.filename, 'latest-win.json');
        assert.equal(metadata.schemaVersion, 1);
        assert.equal(metadata.platform, 'win32');
        assert.equal(metadata.version, '2.7.45');
        assert.equal(metadata.tag, 'v2.7.45');
