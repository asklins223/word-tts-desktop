...
- `pacing.py`：录入速度总控，页面自动化所有人为停顿的唯一参数来源
...
页面录入不能像脚本那样一口气点完：真实页面在字段之间要重渲染、要拉下拉
选项、要回读上传结果。所有停顿集中在一个文件里，调快调慢只改这一处：

`platform_entry/adapter/pacing.py`

- 代码默认值：`PACE_SCALE`（当前 `1.0`）
- 运行期覆盖：`WORDTTS_PLATFORM_INPUT_PACE`，取十进制倍数
  - `0` / `off`：完全不加停顿，回到纯机器速度
  - `0.5`：所有区间减半；`2`：所有区间加倍
  - 无法解析的值按默认档位执行，不会让一次录入因为档位写错而启动失败

`PACE_SCALE=1.0` 时的随机区间（每次取值都在区间内随机，同一条时间线不会
出现两次）：

| 边界 | 含义 | 区间 |
| --- | --- | --- |
| `field` | 一个输入框、富文本字段、下拉选项填好并回读一致之后 | 120–420 ms |
| `item` | 一张题卡 / 课文里一条句子卡片整体填完之后 | 350–900 ms |
| `group` | 切换一个题型栏目、进入下一段段落之前 | 700–1600 ms |
| `step` | 走完一个页面步骤（新增试卷、下一步、保存、一条完整课文）之后 | 900–2000 ms |
| 轮询抖动 | 给 `100`/`200` ms 的等待跳点额外附加 | 30–160 ms |

要单独调某一类边界，就改 `PAUSE_WINDOWS_MS` 里对应那一行；`step` 停顿之后
紧跟停止/暂停检查，所以调慢不会削弱“随时停得下来”。轮询抖动永远不超过
本轮剩余时间，各类等待的 `timeout_seconds` 上限保持不变。

停顿只在动作成功后发生：字段回读不一致、下拉选项找不到时会立刻抛错，不
会先睡一觉再报错。文本上屏方式没有改变（仍然整段一次 `insertText`），速度
差异全部来自边界停顿。

只想看时间花在哪，不想改节奏时，开 `WORDTTS_BROWSER_PERF=1`，标准错误会
多出 `pace:field` / `pace:item` / `pace:group` / `pace:step` 记录，带
`pace_ms` 和当时的档位。音频生成链路（`xunfei/`）不使用这套停顿；只读的
目录同步和只读核验也不停顿。

单元测试里核对停顿次数的用例会用 `patch.dict` 把档位固定成默认值，因此
调用方 shell 里设了 `WORDTTS_PLATFORM_INPUT_PACE` 也不会让测试跟着漂。
