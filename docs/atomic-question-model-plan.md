...
4. ~~旧 UI 命令桥接~~ 已完成（0315801）：解析入口切统一结构读取，主链路解析旁路落原子模型，会话登记 LEGACY_BRIDGED；存量会话由回填工具登记 LEGACY_OUT_OF_BAND；
5. ~~阶段5A 灰度/feature flag/监控~~ **不适用**：产品为全本地化运行
   （Electron + 本地 SQLite），无服务器与运维面；本地质量门 =
   pytest 全量 + `tools/parse_baseline.py compare` 解析基线对照，
   灰度由"按文档/题型逐步启用原子链路"的人工节奏替代。
