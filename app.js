...
function showGenerationRecovery(message) {
    ...
    if (messageEl) {
        messageEl.textContent = message || '生成任务未能继续，请重试或返回调整设置。';
    }
...
function updateGenerationControlUI(workspace = currentWorkspace) {
    ...
    setActionButton('pause-generation-btn', 'PAUSE', state.key === 'PREPARING' || state.key === 'RUNNING' || state.key === 'RECOVERING');
    setActionButton('resume-generation-btn', 'RESUME', state.key === 'PAUSED' || state.key === 'PAUSE_REQUESTED');
    updateConfigActionState(current);
}
...
function adoptAcceptedGeneration(session, snapshot, { reason = '任务已在后台运行' } = {}) {
    ...
    $('generation-live-status').textContent = stopping ? '正在结束本地任务…' : reason;
...
function workspaceNavigationLockReason(workspaceName) {
    ...
    if (workspaceName === 'generation') return '开始生成后才能进入生成任务';
